...
export default function Home(){
  ...
  return <div className="shell">
    ...
    <aside className={menu?"side show":"side"}>
      ...
      <div className="brand"><i>Dİ</i><div><b>Daxili İdarəetmə</b><small>İş və tapşırıq sistemi</small><small className="brandversion">Versiya 1.67</small></div></div>
...
function TaskGrid({tasks,employeeView,onStatus,onEvaluate,onDelete,dateRequests,onRequestDate,onResolveDateRequest}:{tasks:Task[];employeeView:boolean;onStatus:(t:Task,s:string,extra?:Record<string,unknown>)=>void;onEvaluate:(t:Task)=>void;onDelete:(t:Task)=>void;dateRequests:DateRequest[];onRequestDate:(taskId:number,proposedDueAt:string,reason:string)=>void;onResolveDateRequest:(id:number,approve:boolean,adminNote:string,finalDueAt:string)=>void}){
  ...
  const taskColumns:Array<{key:string;label:string;width:number;search:(t:Task)=>string;render:(t:Task)=>React.ReactNode}>=[
    ...
    {key:"evaluation",label:"Qiymətləndirmə",width:160,search:t=>t.evaluation?`${t.evaluation} bal`:displayStatus(t),render:t=><div className="tableactions"><RatingCell evaluation={t.evaluation} note={t.evaluation_note} compact twoRows numeric={!employeeView}/>{!employeeView&&t.status==="Təqdim edilib"?<button className="evaluatebtn" onClick={()=>onEvaluate(t)}>Qiymətləndir</button>:!t.evaluation&&!t.evaluation_note&&<span>—</span>}{!employeeView&&t.status==="Yeni"&&<button className="deletetaskbtn" onClick={()=>onDelete(t)}>Sil</button>}</div>},
  ];
...
function RatingCell({evaluation,note,compact=false,twoRows=false,numeric=false}:{evaluation:number|null;note:string|null;compact?:boolean;twoRows?:boolean;numeric?:boolean}){
  if(!evaluation&&!note)return null;
  const content=evaluation?(numeric?<span className={`ratingpill ${scoreTier(evaluation)}`}>{evaluation}<small>/10</small></span>:<RatingStars value={evaluation} compact={compact} twoRows={twoRows}/>):<span className="nodocument">—</span>;
...
function ChecklistSection({employeeView,checklist,loading,title,setTitle,busy,error,onAdd,onToggle,onRemove,canDelegate=false,delegateEmployees=[],onDelegate,onAttach,onDetach,attachBusyId=null,locked=false}:{employeeView:boolean;checklist:ChecklistLikeItem[];loading:boolean;title:string;setTitle:(v:string)=>void;busy:boolean;error:string;onAdd:()=>void;onToggle:(item:ChecklistLikeItem)=>void;onRemove:(item:ChecklistLikeItem)=>void;canDelegate?:boolean;delegateEmployees?:Employee[];onDelegate?:(item:ChecklistLikeItem,employeeId:string,comment:string)=>void;onAttach?:(item:ChecklistLikeItem,file:File)=>void;onDetach?:(item:ChecklistLikeItem)=>void;attachBusyId?:number|null;locked?:boolean}){
    ...
    {loading?<small>Yüklənir...</small>:<>
      {checklist.length?<ul className="checklist">{checklist.map(item=><li key={item.id} className={item.done?"done":""}>
        ...
        <div className="checklistitemactions">
          ...
          {item.delegated_task_id?<small className="delegatedtag">Həvalə edilib: {item.delegated_employee_name||"—"} — {item.delegated_task_status||"Yeni"}</small>:<>
          {canDelegate&&<span className="checklistdelegate"><select disabled={locked} value={choice[item.id]||""} onChange={e=>setChoice({...choice,[item.id]:e.target.value})}><option value="">İşçi seçin</option>{delegateEmployees.map(emp=><option key={emp.id} value={emp.id}>{emp.name}</option>)}</select><button type="button" className="delegatebtn" disabled={locked||!choice[item.id]} onClick={()=>{setPending({itemId:item.id,employeeId:choice[item.id]});setComment("")}}>Ver</button></span>}
