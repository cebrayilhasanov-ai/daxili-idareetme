...
type Employee = { id:number; name:string; position:string; email:string|null; active:number; created_at:string; company_ids:string|null; avatar_key:string|null; manager_employee_id:number|null; authority_type:string };
const authorityTypes=["Direktor","Şöbə müdiri","İşçi"];
...
export default function Home(){
  ...
  const saveEmployeeEdit=async()=>{setError("");setUploadingPhoto(Boolean(employeePhoto));try{const avatarKey=await uploadPhotoIfAny();await request("PATCH",{action:"employee",id:Number(form.id),name:form.name,position:form.position,email:form.email,companyIds:(form.companyIds||"").split(",").filter(Boolean).map(Number),managerEmployeeId:form.managerEmployeeId?Number(form.managerEmployeeId):null,authorityType:form.authorityType||"İşçi",...(avatarKey?{avatarKey}:{})});setEmployeePhoto(null)}catch(e){setError(e instanceof Error?e.message:"Personal yenilənmədi.")}finally{setUploadingPhoto(false)}};
  ...
  // Non-admins working across several firms pick one active firma from the sidebar; every company-linked section (tasks, works, HR, fixed works) then scopes to it.
  const myCompanies=isAdmin?[]:data.companies.filter(c=>Boolean(c.active));
  useEffect(()=>{
    if(isAdmin||!user)return;
    let saved:number|null=null;
    try{const raw=localStorage.getItem(`activeCompany:${user.id}`);saved=raw?Number(raw):null}catch{saved=null}
  ...
  },[isAdmin,user?.id,myCompanies.map(c=>c.id).join(",")]);
  const pickCompany=(id:number)=>{setActiveCompanyId(id);if(user)try{localStorage.setItem(`activeCompany:${user.id}`,String(id))}catch{}};
  const visibleTasks=(effectiveView?data.tasks.filter(t=>t.employee_id===effectiveView.id):data.tasks).filter(t=>isAdmin||!activeCompanyId||t.company_id===activeCompanyId);
  ...
  return <div className="shell">
    ...
    <aside className={menu?"side show":"side"}>
      ...
      <div className="brand"><i>Dİ</i><div><b>Daxili İdarəetmə</b><small>İş və tapşırıq sistemi</small><small className="brandversion">Versiya 1.90</small></div></div>
      {!isAdmin&&myCompanies.length>1&&<div className="companyswitcher"><label>Aktiv firma<select value={activeCompanyId??""} onChange={e=>pickCompany(Number(e.target.value))}>{myCompanies.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label></div>}
...
function TasksPage({employeeView,tasks,onDelete,onStatus,onEvaluate,dateRequests,onRequestDate,onResolveDateRequest,employees,isAdmin,currentEmployeeId}:{employeeView:boolean;tasks:Task[];onDelete:(t:Task)=>void;onStatus:(t:Task,s:string,extra?:Record<string,unknown>)=>void;onEvaluate:(t:Task)=>void;dateRequests:DateRequest[];onRequestDate:(taskId:number,proposedDueAt:string,reason:string)=>void;onResolveDateRequest:(id:number,approve:boolean,adminNote:string,finalDueAt:string)=>void;employees:Employee[];isAdmin:boolean;currentEmployeeId:number|null}){
  return <section className="panel pagepanel">
    ...
    <TaskGrid tasks={tasks} employeeView={employeeView} onStatus={onStatus} onEvaluate={onEvaluate} onDelete={onDelete} dateRequests={dateRequests} onRequestDate={onRequestDate} onResolveDateRequest={onResolveDateRequest} employees={employees} isAdmin={isAdmin} currentEmployeeId={currentEmployeeId}/>
  </section>
}
function PersonalWorksPage({isAdmin,currentUserId,viewAsEmployeeId,companies,employees,activeCompanyId}:{isAdmin:boolean;currentUserId:number;viewAsEmployeeId?:number|null;companies:Company[];employees:Employee[];activeCompanyId?:number|null}){
  ...
  const filtered=items.filter(item=>(isAdmin||!activeCompanyId||item.company_id===activeCompanyId)&&personalWorkColumns.every(c=>has(c.search(item),c.key)));
  ...
  const delegateEligibleEmployees=current?.company_id?employees.filter(e=>(e.company_ids||"").split(",").filter(Boolean).map(Number).includes(current.company_id as number)):[];
...
function TaskGrid({tasks,employeeView,onStatus,onEvaluate,onDelete,dateRequests,onRequestDate,onResolveDateRequest,employees,isAdmin,currentEmployeeId}:{tasks:Task[];employeeView:boolean;onStatus:(t:Task,s:string,extra?:Record<string,unknown>)=>void;onEvaluate:(t:Task)=>void;onDelete:(t:Task)=>void;dateRequests:DateRequest[];onRequestDate:(taskId:number,proposedDueAt:string,reason:string)=>void;onResolveDateRequest:(id:number,approve:boolean,adminNote:string,finalDueAt:string)=>void;employees:Employee[];isAdmin:boolean;currentEmployeeId:number|null}){
  ...
  const canDelegateTask=isAdmin||Boolean(currentEmployeeId&&current&&current.employee_id===currentEmployeeId);
  const delegateEligibleEmployees=current?.company_id?employees.filter(e=>(e.company_ids||"").split(",").filter(Boolean).map(Number).includes(current.company_id as number)&&(isAdmin||e.manager_employee_id===current.employee_id)):[];
...
function historyTone(action:string){
  ...
  if(a.includes("silindi")||a.includes("ləğv")||a.includes("geri qaytarıldı")||a.includes("götürüldü"))return "bad";
  if(a.includes("tamamlandı")||a.includes("təsdiqləndi"))return "good";
  if(a.includes("işçi"))return "share";
  if(a.includes("fayl"))return "file";
  return "info";
}
function WorkHistory({events}:{events:WorkHistoryEvent[]|null}){
  ...
  return <div className="historysection"><div className="historyhead"><b>İşin tarixçəsi</b><span className="historyheadright">{events&&<small>{events.length} hadisə</small>}<button type="button" className="historytoggle" aria-expanded={open} onClick={toggle}>{open?"Bağla":"Aç"}</button></span></div>
    ...
    {loading?<small>Yüklənir...</small>:<>
      {checklist.length?<ul className="checklist">{checklist.map(item=><li key={item.id} className={item.done?"done":""}>
        ...
        <div className="checklistitemactions">
          ...
          {canDelegate&&<span className="checklistdelegate"><select disabled={locked||!stepsActionable} value={choice[item.id]||""} onChange={e=>setChoice({...choice,[item.id]:e.target.value})}><option value="">İşçi seçin</option>{delegateEmployees.map(emp=><option key={emp.id} value={emp.id}>{emp.name}</option>)}</select><button type="button" className="delegatebtn" disabled={locked||!stepsActionable||!choice[item.id]} onClick={()=>{setPending({itemId:item.id,employeeId:choice[item.id]});setComment("")}}>Ver</button></span>}
