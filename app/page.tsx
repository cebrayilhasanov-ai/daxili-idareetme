...
export default function Home(){
  ...
  return <div className="shell">
    ...
    <aside className={menu?"side show":"side"}>
      ...
      <div className="brand"><i>Dİ</i><div><b>Daxili İdarəetmə</b><small>İş və tapşırıq sistemi</small><small className="brandversion">Versiya 1.27</small></div></div>
...
function CustomersPage(){
  ...
  return <section className="panel pagepanel directorypanel">
    ...
    {error&&<div className="errorbox">{error}</div>}
    {loading?<div className="loading">Yüklənir...</div>:<div className="tasktablewrap"><table className="tasktable documenttable customertable"><ColGroup defaults={[130,100,200,340,160,140]} widths={colWidths}/><thead><tr><th {...resize(0)}><input aria-label="Statusu üzrə axtarış" placeholder="Axtar..." value={search.status||""} onChange={e=>setQuery("status",e.target.value)}/><span>Statusu</span></th><th {...resize(1)}><input aria-label="VÖEN/FİN üzrə axtarış" placeholder="Axtar..." value={search.voen||""} onChange={e=>setQuery("voen",e.target.value)}/><span>VÖEN/FİN</span></th><th {...resize(2)}><input aria-label="Müştərinin adı üzrə axtarış" placeholder="Axtar..." value={search.name||""} onChange={e=>setQuery("name",e.target.value)}/><span>Müştərinin adı</span></th><th {...resize(3)}><input aria-label="Hüquqi ünvan üzrə axtarış" placeholder="Axtar..." value={search.address||""} onChange={e=>setQuery("address",e.target.value)}/><span>Hüquqi ünvan</span></th><th {...resize(4)}><input aria-label="Rəhbər üzrə axtarış" placeholder="Axtar..." value={search.manager||""} onChange={e=>setQuery("manager",e.target.value)}/><span>Rəhbər</span></th><th {...resize(5)} className={`opencolumn${resize(5).className?` ${resize(5).className}`:""}`}><span>Əməliyyat</span></th></tr></thead><tbody>{filtered.map(item=>editingId===item.id?<tr key={item.id}><td colSpan={6}><div className="inlinetaskrow documentrow customerrow documenteditrow">{fields(editForm,setEditForm)}<div className="inlineactions"><button className="inlinecancel" disabled={editBusy} onClick={cancelEdit}>Ləğv et</button><Button disabled={editBusy||!requiredFilled(editForm)} onClick={()=>void saveEdit(item.id)}>{editBusy?"Yadda saxlanılır...":"Yadda saxla"}</Button></div></div></td></tr>:<tr key={item.id}>
