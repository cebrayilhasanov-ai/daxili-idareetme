...
export default function Home(){
  ...
  // Whoever is the "active identity" (a real non-admin login, OR an admin using Personal görünüşü — including on themselves) picks one active firma from the
  // company cards on the dashboard; every company-linked section (tasks, works, HR, fixed works) then scopes to it. Plain admin (no viewAs) stays unscoped, seeing every firma.
  const companyScopeActive=Boolean(effectiveView);
  // Main workplace first (it is also the default active firma), then the rest by name.
  const myCompanies=effectiveView?data.companies.filter(c=>Boolean(c.active)&&(effectiveView.company_ids||"").split(",").filter(Boolean).map(Number).includes(c.id)).sort((a,b)=>Number(b.id===effectiveView.main_company_id)-Number(a.id===effectiveView.main_company_id)||a.name.localeCompare(b.name,"az")):[];
  // Card badge: tasks in that firma still waiting in "Yeni" (received, not yet taken into work).
  const newTaskCounts:Record<number,number>=Object.fromEntries(myCompanies.map(c=>[c.id,data.tasks.filter(t=>t.employee_id===effectiveView?.id&&t.company_id===c.id&&t.status==="Yeni").length]));
  useEffect(()=>{
  ...
  return <div className="shell">
    ...
    <aside className={menu?"side show":"side"}>
      ...
      <div className="brand"><i>Dİ</i><div><b>Daxili İdarəetmə</b><small>İş və tapşırıq sistemi</small><small className="brandversion">Versiya 2.17</small></div></div>
...
function Dashboard({userName,avatarKey,onEditAvatar,tasks,active,overdue,completed,employees,goTasks,evaluationEmployees,ownEmployeeId,activeCompanyId,companies,mainCompanyId,newTaskCounts,onPickCompany}:{userName:string;avatarKey:string|null;onEditAvatar:()=>void;tasks:Task[];active:number;overdue:number;completed:number;employees:number;goTasks:()=>void;evaluationEmployees:Employee[];ownEmployeeId:number|null;activeCompanyId?:number|null;companies:Company[];mainCompanyId:number|null;newTaskCounts:Record<number,number>;onPickCompany:(id:number)=>void}){return <><section className="welcome"><div><small>{new Intl.DateTimeFormat("az-AZ",{day:"2-digit",month:"long",year:"numeric"}).format(new Date()).toUpperCase()}</small><h2>Salam, {userName}</h2><p>Bu gün komandanızın iş vəziyyətini buradan izləyə bilərsiniz.</p></div><div className="welcomeaside"><button className={avatarKey?"welcomephotobtn":"welcomephotobtn empty"} title={avatarKey?"Profil şəklini dəyiş":"Profil şəkli əlavə et"} onClick={onEditAvatar}>{avatarKey?<img className="welcomephoto" src={`/api/file?key=${encodeURIComponent(avatarKey)}`} alt={userName}/>:<span className="welcomephoto">{initials(userName)}</span>}</button><Bell/></div></section>{companies.length>0&&<section className="companycards" aria-label="İşlədiyim firmalar">{companies.map(c=>{const count=newTaskCounts[c.id]||0;return <button key={c.id} type="button" className={c.id===activeCompanyId?"companycard on":"companycard"} aria-pressed={c.id===activeCompanyId} onClick={()=>onPickCompany(c.id)}><small>{c.id===mainCompanyId?"Əsas iş yeri":"Firma"}</small><b>{c.name}</b>{count>0&&<em title={`${count} yeni tapşırıq`} aria-label={`${count} yeni tapşırıq`}>{count}</em>}</button>})}</section>}<EvaluationSection employees={evaluationEmployees} tasks={tasks}/><section className="stats"><Stat icon={<ClipboardList/>} tone="blue" label="Aktiv tapşırıq" value={active}/><Stat icon={<CircleAlert/>} tone="red" label="Gecikən" value={overdue}/><Stat icon={<CheckCircle2/>} tone="green" label="Tamamlanan" value={completed}/><Stat icon={<Users/>} tone="gold" label="Aktiv personal" value={employees}/></section>{ownEmployeeId&&<MyViolationsPanel employeeId={ownEmployeeId} activeCompanyId={activeCompanyId}/>}<div className="modulecharts"><TaskStatusChart tasks={tasks} onViewAll={goTasks}/><DocumentsOverviewChart/><ViolationsChart/></div></>}
