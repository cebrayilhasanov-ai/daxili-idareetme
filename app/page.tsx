...
type ChecklistLikeItem = { id:number; title:string; done:number; delegated_task_id?:number|null; delegated_employee_name?:string|null; delegated_task_status?:string|null };
type PersonalWork = { id:number; user_id:number; owner_name:string; title:string; description:string|null; company_id:number|null; company_name:string|null; due_at:string|null; status:string; created_at:string; completed_at:string|null; attachment_key:string|null; attachment_name:string|null; attachment_size:number|null; attachment_type:string|null; delegated_task_id:number|null; delegated_employee_id:number|null; delegated_employee_name:string|null; delegated_task_status:string|null };
type PersonalWorkChecklistItem = { id:number; personal_work_id:number; title:string; done:number; created_at:string; delegated_task_id:number|null; delegated_employee_id:number|null; delegated_employee_name:string|null; delegated_task_status:string|null };
...
export default function Home(){
  ...
  return <div className="shell">
    ...
    <aside className={menu?"side show":"side"}>
      ...
      <div className="brand"><i>Dİ</i><div><b>Daxili İdarəetmə</b><small>İş və tapşırıq sistemi</small><small className="brandversion">Versiya 1.42</small></div></div>
...
function PersonalWorksPage({isAdmin,currentUserId,companies,employees}:{isAdmin:boolean;currentUserId:number;companies:Company[];employees:Employee[]}){
  ...
  const [delegateChoice,setDelegateChoice]=useState("");
  ...
  const openDetail=(item:PersonalWork)=>{setDetailItem(item);setDelegateChoice("")};
  ...
  const delegateWork=async(item:PersonalWork,employeeId:string)=>{
    if(!employeeId)return;
    setError("");
    try{
      const response=await fetch("/api/personal-works",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({id:item.id,delegateEmployeeId:employeeId})});
      const body=await response.json();
      if(!response.ok)throw new Error(body.error);
      setItems(body.items||[]);
    }catch(e){setError(e instanceof Error?e.message:"Həvalə edilmədi.")}
  };
  const delegateChecklistItem=async(item:ChecklistLikeItem,employeeId:string)=>{
    if(!employeeId)return;
    setChecklistError("");
    try{
      const response=await fetch("/api/personal-work-checklist",{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({id:item.id,delegateEmployeeId:employeeId})});
      const body=await response.json();
      if(!response.ok)throw new Error(body.error);
  ...
  return <section className="panel pagepanel">
    ...
    <Dialog open={Boolean(current)} onOpenChange={v=>!v&&setDetailItem(null)}><DialogContent className="businessdialog">{current&&<FormShell title={current.title} desc={currentOwn?"Öz işim":current.owner_name} formClass="taskdetailform"><div className="taskdetailleft"><div className="taskdetailinfo"><p><b>Açıqlama</b><span>{current.description||"—"}</span></p><p><b>Firma</b><span>{current.company_name||"—"}</span></p><p><b>Yaranma tarixi</b><span>{formatDate(current.created_at)}</span></p><p><b>Son tarix</b><span>{current.due_at?formatDate(current.due_at):"—"}</span></p>{current.attachment_key&&<p><b>Əlavə olunan sənəd</b><span><a className="filelink" href={`/api/file?key=${encodeURIComponent(current.attachment_key)}`}>{current.attachment_name}<small>{formatFileSize(current.attachment_size||0)}</small></a></span></p>}</div><div className="field"><span>Status</span><strong className={`detailstatus ${statusTone(current.status)}`}>{current.status}</strong>{currentOwn&&current.status!=="Tamamlanıb"&&!current.delegated_task_id&&<Button onClick={()=>void advance(current)}>{current.status==="Yeni"?"İcraya al":"Tamamla"}</Button>}</div>{current.delegated_task_id&&<small className="delegatedtag">Həvalə edilib: {current.delegated_employee_name} — {current.delegated_task_status||"Yeni"}</small>}{isAdmin&&currentOwn&&!current.delegated_task_id&&current.status!=="Tamamlanıb"&&<div className="field delegatework"><span>Bu işi işçiyə həvalə et</span>{current.company_id?<div className="checklistdelegate"><select value={delegateChoice} onChange={e=>setDelegateChoice(e.target.value)}><option value="">İşçi seçin</option>{delegateEligibleEmployees.map(emp=><option key={emp.id} value={emp.id}>{emp.name}</option>)}</select><button type="button" className="delegatebtn" disabled={!delegateChoice||!current.due_at} onClick={()=>void delegateWork(current,delegateChoice)}>Həvalə et</button></div>:<small>Həvalə etmək üçün əvvəlcə firma seçin.</small>}{current.company_id&&!current.due_at&&<small className="expirednote">Həvalə etmək üçün son tarix təyin edin.</small>}</div>}{currentOwn&&current.status==="Yeni"&&!current.delegated_task_id&&<button className="deletetaskbtn detaildelete" onClick={()=>{setDetailItem(null);void remove(current)}}>İşi sil</button>}</div><div className="taskdetailright"><ChecklistSection employeeView={currentOwn} checklist={checklist} loading={checklistLoading} title={checklistTitle} setTitle={setChecklistTitle} busy={checklistBusy} error={checklistError} onAdd={()=>void addChecklistItem()} onToggle={item=>void toggleChecklistDone(item)} onRemove={item=>void removeChecklistItem(item)} canDelegate={isAdmin&&currentOwn} delegateEmployees={delegateEligibleEmployees} onDelegate={(item,employeeId)=>void delegateChecklistItem(item,employeeId)}/></div></FormShell>}</DialogContent></Dialog>
  </section>;
}
...
function EvaluationSection({employees,tasks}:{employees:Employee[];tasks:Task[]}){
  ...
  return <section className="panel evalpanel"><div className="head"><div><h3>Qiymətləndirmə</h3><p>Təsdiqlənmiş işlər üzrə nəticələr</p></div></div><div className="evaluationrows">{employees.map((e,index)=>{
    ...
    const tier=avgNum>=8?"":avgNum>=5?"mid":"low";
...
function ChecklistSection({employeeView,checklist,loading,title,setTitle,busy,error,onAdd,onToggle,onRemove,canDelegate=false,delegateEmployees=[],onDelegate}:{employeeView:boolean;checklist:ChecklistLikeItem[];loading:boolean;title:string;setTitle:(v:string)=>void;busy:boolean;error:string;onAdd:()=>void;onToggle:(item:ChecklistLikeItem)=>void;onRemove:(item:ChecklistLikeItem)=>void;canDelegate?:boolean;delegateEmployees?:Employee[];onDelegate?:(item:ChecklistLikeItem,employeeId:string)=>void}){
  ...
  return <div className="checklistsection"><div className="checklisthead"><b>Mənim iş axınım</b>{checklist.length>0&&<small>{done}/{checklist.length} tamamlandı <em>({pct}%)</em></small>}</div>
    {loading?<small>Yüklənir...</small>:<>
      {checklist.length?<ul className="checklist">{checklist.map(item=><li key={item.id} className={item.done?"done":""}>
        <label><input type="checkbox" checked={Boolean(item.done)} disabled={!employeeView||Boolean(item.delegated_task_id)} onChange={()=>onToggle(item)}/><span>{item.title}</span></label>
        {item.delegated_task_id?<small className="delegatedtag">Həvalə edilib: {item.delegated_employee_name} — {item.delegated_task_status||"Yeni"}</small>:<div className="checklistitemactions">
          {canDelegate&&<span className="checklistdelegate"><select value={choice[item.id]||""} onChange={e=>setChoice({...choice,[item.id]:e.target.value})}><option value="">İşçi seçin</option>{delegateEmployees.map(emp=><option key={emp.id} value={emp.id}>{emp.name}</option>)}</select><button type="button" className="delegatebtn" disabled={!choice[item.id]} onClick={()=>{onDelegate?.(item,choice[item.id]);setChoice({...choice,[item.id]:""})}}>Ver</button></span>}
