...
export default function Home(){
  ...
  return <div className="shell">
    ...
    <aside className={menu?"side show":"side"}>
      ...
      <div className="brand"><i>Dİ</i><div><b>Daxili İdarəetmə</b><small>İş və tapşırıq sistemi</small><small className="brandversion">Versiya 1.24</small></div></div>
...
function CustomersPage(){
  ...
  return <section className="panel pagepanel directorypanel">
    ...
    {creating&&<div className="inlinetaskrow documentrow customerrow">{fields(form,setForm)}<div className="inlineactions"><button className="inlinecancel" disabled={busy} onClick={()=>{setCreating(false);setForm({})}}>Ləğv et</button><Button disabled={busy||!(form.name||"").trim()} onClick={()=>void create()}>{busy?"Yaradılır...":"Əlavə et"}</Button></div></div>}
    {error&&<div className="errorbox">{error}</div>}
    {loading?<div className="loading">Yüklənir...</div>:<div className="tasktablewrap"><table className="tasktable documenttable customertable"><ColGroup defaults={[130,100,200,340,160,140]} widths={colWidths}/><thead><tr><th {...resize(0)}><span>Statusu</span></th><th {...resize(1)}><span>VÖEN</span></th><th {...resize(2)}><span>Müştərinin adı</span></th><th {...resize(3)}><span>Hüquqi ünvan</span></th><th {...resize(4)}><span>Rəhbər</span></th><th {...resize(5)} className={`opencolumn${resize(5).className?` ${resize(5).className}`:""}`}><span>Əməliyyat</span></th></tr></thead><tbody>{items.map(item=>editingId===item.id?<tr key={item.id}><td colSpan={6}><div className="inlinetaskrow documentrow customerrow documenteditrow">{fields(editForm,setEditForm)}<div className="inlineactions"><button className="inlinecancel" disabled={editBusy} onClick={cancelEdit}>Ləğv et</button><Button disabled={editBusy||!(editForm.name||"").trim()} onClick={()=>void saveEdit(item.id)}>{editBusy?"Yadda saxlanılır...":"Yadda saxla"}</Button></div></div></td></tr>:<tr key={item.id}>
