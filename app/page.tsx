...
import { MONTH_NAMES, WEEKDAY_NAMES, bakuToday, dueDay, dueLabel, dueMonthOffset, formatBakuDate, monthlyKey, overdueDays, periodState, periodWindow, weeksOfMonth } from "@/lib/fixed-periods";
...
type WorkItem = { id:number; title:string; description:string|null; frequency:"monthly"|"weekly"|"daily"; company_ids:string|null; due_day:number|null; due_month_offset:number|null };
type WorkAssignment = { id:number; work_definition_id:number; title:string; description:string|null; frequency:"monthly"|"weekly"|"daily"; due_day:number|null; due_month_offset:number|null; employee_id:number; employee_name:string; company_id:number; company_name:string; period_key:string; is_completed:number };
...
export default function Home(){
  ...
  return <div className="shell">
    ...
    <aside className={menu?"side show":"side"}>
      ...
      <div className="brand"><i>Dİ</i><div><b>Daxili İdarəetmə</b><small>İş və tapşırıq sistemi</small><small className="brandversion">Versiya 2.14</small></div></div>
...
function WorkList({employeeView,tab,frequency,items,assignments,completions,employees,companies,form,setForm,onAdd,onDue,onAssign,onCatalogAssign,onComplete}:{employeeView:boolean;tab:"catalog"|"assignments";frequency:"monthly"|"weekly";items:WorkItem[];assignments:WorkAssignment[];completions:WorkCompletion[];employees:Employee[];companies:Company[];form:Record<string,string>;setForm:React.Dispatch<React.SetStateAction<Record<string,string>>>;onAdd:()=>void;onDue:(item:WorkItem,dueDay:number,dueMonthOffset:number|null)=>void;onAssign:()=>void;onCatalogAssign:(workDefinitionId:number,companyId:number,employeeId:number)=>void;onComplete:(assignmentId:number,periodKey:string)=>void}){const [creating,setCreating]=useState(false);const [catalogCompanyFilter,setCatalogCompanyFilter]=useState("all");const [catalogEmployeeFilter,setCatalogEmployeeFilter]=useState("all");
  ...
  const shownCompanies=catalogCompanyFilter==="all"?companies:companies.filter(c=>String(c.id)===catalogCompanyFilter);const assignmentView=employeeView||tab==="assignments";const [catalogWidths,setCatalogWidth]=useSimpleColumnWidths("workcatalog3");const resizeCatalog=useEdgeResize(setCatalogWidth,40);const gridTemplate=(defaults:string[],widths:Record<string,number>)=>defaults.map((d,i)=>widths[i]?`${widths[i]}px`:d).join(" ");const catalogColumns={gridTemplateColumns:gridTemplate(["56px","minmax(240px,1fr)","minmax(300px,1.2fr)","190px",...shownCompanies.map(()=>"92px")],catalogWidths)};const selectedCompanies=new Set((form.assignCompanyIds||"").split(",").filter(Boolean).map(Number));const toggleAssignCompany=(id:number,checked:boolean)=>{const next=new Set(selectedCompanies);checked?next.add(id):next.delete(id);setForm({...form,assignCompanyIds:[...next].join(",")})};const filteredCatalogItems=items.filter(item=>(catalogCompanyFilter==="all"&&catalogEmployeeFilter==="all")||assignments.some(a=>a.work_definition_id===item.id&&(catalogCompanyFilter==="all"||String(a.company_id)===catalogCompanyFilter)&&(catalogEmployeeFilter==="all"||String(a.employee_id)===catalogEmployeeFilter)));
  ...
  const dueEditor=(item:WorkItem)=>item.frequency==="weekly"
    ?<label className="catalogdue"><select aria-label={`${item.title}: son tarix`} value={dueDay(item)} onChange={e=>onDue(item,Number(e.target.value),null)}>{WEEKDAY_NAMES.map((name,i)=><option key={name} value={i+1}>{name}</option>)}</select></label>
    :<label className="catalogdue"><select aria-label={`${item.title}: son tarixin günü`} value={dueDay(item)} onChange={e=>onDue(item,Number(e.target.value),dueMonthOffset(item))}>{Array.from({length:31},(_,i)=><option key={i+1} value={i+1}>{i+1}</option>)}</select><select aria-label={`${item.title}: son tarixin ayı`} value={dueMonthOffset(item)} onChange={e=>onDue(item,dueDay(item),Number(e.target.value))}><option value={1}>növbəti ay</option><option value={0}>həmin ay</option></select></label>;
  ...
  const periods=frequency==="monthly"?MONTH_NAMES.map((label,i)=>({key:monthlyKey(year,i),label})):weeksOfMonth(year,month);
  const periodColumns={gridTemplateColumns:["48px","minmax(200px,1fr)","minmax(200px,1fr)",...(showCompany?["150px"]:[]),...(employeeView?[]:["150px"]),"130px",...periods.map(()=>frequency==="monthly"?"78px":"104px")].join(" ")};
  ...
  const periodGroup=()=>rows.length?<><div className="workmatrix periodmatrix"><div className="workmatrixhead" style={periodColumns}><span>№</span><span>İşlərin siyahısı</span><span>İşin açıqlaması</span>{showCompany&&<span>Firma</span>}{!employeeView&&<span>İstifadəçi</span>}<span>Son tarix</span>{periods.map(p=><span key={p.key}>{p.label}</span>)}</div>{rows.map((a,index)=><article key={a.id} style={periodColumns}><b className="rownumber">{index+1}</b><h3>{a.title}</h3><p className="workdescription">{a.description||"—"}</p>{showCompany&&<span className="matrixuser">{a.company_name}</span>}{!employeeView&&<span className="matrixuser">{a.employee_name}</span>}<span className="matrixuser">{dueLabel(a)}</span>{periods.map(p=>periodCell(a,p))}</article>)}</div><div className="periodlegend"><span><i className="periodcell active"/>Açıqdır — icra etmək vaxtıdır</span><span><i className="periodcell done"/>Vaxtında icra edilib</span><span><i className="periodcell late-done"/>Gecikməklə icra edilib</span><span><i className="periodcell overdue"/>Gecikib</span><span><i className="periodcell future"/>Hələ açılmayıb</span></div></>:<Empty text={employeeView?"Sizə hələ sabit iş təyin edilməyib.":"Hələ personala sabit iş təyin edilməyib."}/>;
