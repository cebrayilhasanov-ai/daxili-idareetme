...
type WorkHistoryEvent = { id:number; actor_name:string; action:string; detail:string|null; created_at:string };
...
export default function Home(){
  ...
  return <div className="shell">
    ...
    <aside className={menu?"side show":"side"}>
      ...
      <div className="brand"><i>Dİ</i><div><b>Daxili İdarəetmə</b><small>İş və tapşırıq sistemi</small><small className="brandversion">Versiya 1.60</small></div></div>
...
function WorkHistory({events}:{events:WorkHistoryEvent[]|null}){
  return <div className="historysection"><div className="historyhead"><b>İşin tarixçəsi</b>{events&&<small>{events.length} hadisə</small>}</div>
    {!events?<small>Yüklənir...</small>:<ol className="historylist">{events.map((event,index)=><li key={`${event.id}-${index}`} className={`tone-${historyTone(event.action)}`}><i/><div className="historybody"><div className="historyrow"><b>{event.action}</b><time>{formatDate(event.created_at)}</time></div><span className="historyactor">{event.actor_name}</span>{event.detail&&<p>{event.detail}</p>}</div></li>)}</ol>}
  </div>;
}
