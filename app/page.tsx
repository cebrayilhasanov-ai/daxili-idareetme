...
export default function Home(){
  ...
  return <div className="shell">
    ...
    <aside className={menu?"side show":"side"}>
      ...
      <div className="brand"><i>Dİ</i><div><b>Daxili İdarəetmə</b><small>İş və tapşırıq sistemi</small><small className="brandversion">Versiya 1.57</small></div></div>
    ...
    {loading?<small>Yüklənir...</small>:<>
      {checklist.length?<ul className="checklist">{checklist.map(item=><li key={item.id} className={item.done?"done":""}>
        ...
        <div className="checklistitemactions">
          ...
          {canDelegate&&<span className="checklistdelegate"><select disabled={locked} value={choice[item.id]||""} onChange={e=>setChoice({...choice,[item.id]:e.target.value})}><option value="">İşçi seçin</option>{delegateEmployees.map(emp=><option key={emp.id} value={emp.id}>{emp.name}</option>)}</select><button type="button" className="delegatebtn" disabled={locked||!choice[item.id]} onClick={()=>{onDelegate?.(item,choice[item.id]);setChoice({...choice,[item.id]:""})}}>Ver</button></span>}
