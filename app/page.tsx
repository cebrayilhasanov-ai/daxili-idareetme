...
export default function Home(){
  ...
  return <div className="shell">
    ...
    <aside className={menu?"side show":"side"}>
      ...
      <div className="brand"><i>Dİ</i><div><b>Daxili İdarəetmə</b><small>İş və tapşırıq sistemi</small><small className="brandversion">Versiya 1.39</small></div></div>
...
function OutgoingDocumentsPage({isAdmin}:{isAdmin:boolean}){
  ...
  const templateAndFileBlock=(values:Record<string,string>,fileValue:File|null,setFileValue:(f:File|null)=>void)=>{
    ...
    return <>
      {matched&&<div className="templateusepanel"><b>Şablondan istifadə et</b><p>Şablonu yükləyib doldurun, sonra aşağıdan hazır sənədi əlavə edin.</p><div className="templateuseslots">{templateSlots.map(t=><div className="templateuseslot" key={t.n}><span className="templatelabel">Sənədin şablonu<br/>{t.n}</span>{t.key?<div className="templateuseactions"><a className="filelink" href={`/api/file?key=${encodeURIComponent(t.key)}`} target="_blank" rel="noreferrer">{t.name}</a><a className="viewbtn" href={`/api/file?key=${encodeURIComponent(t.key)}&view=1`} target="_blank" rel="noreferrer">Baxış</a></div>:<span className="nodocument">Yoxdur</span>}</div>)}</div></div>}
