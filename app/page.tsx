...
export default function Home(){
  ...
  return <div className="shell">
    ...
    <aside className={menu?"side show":"side"}>
      ...
      <div className="brand"><i>Dİ</i><div><b>Daxili İdarəetmə</b><small>İş və tapşırıq sistemi</small><small className="brandversion">Versiya 2.13</small></div></div>
...
function WorkList({employeeView,tab,frequency,items,assignments,employees,companies,form,setForm,onAdd,onFrequency,onAssign,onCatalogAssign,onComplete}:{employeeView:boolean;tab:"catalog"|"assignments";frequency:"monthly"|"weekly";items:WorkItem[];assignments:WorkAssignment[];employees:Employee[];companies:Company[];form:Record<string,string>;setForm:React.Dispatch<React.SetStateAction<Record<string,string>>>;onAdd:()=>void;onFrequency:(item:WorkItem,frequency:"monthly"|"weekly"|"daily")=>void;onAssign:()=>void;onCatalogAssign:(workDefinitionId:number,companyId:number,employeeId:number)=>void;onComplete:(assignmentId:number)=>void}){const [creating,setCreating]=useState(false);const [catalogCompanyFilter,setCatalogCompanyFilter]=useState("all");const [catalogEmployeeFilter,setCatalogEmployeeFilter]=useState("all");
  ...
  const shownCompanies=catalogCompanyFilter==="all"?companies:companies.filter(c=>String(c.id)===catalogCompanyFilter);const assignmentView=employeeView||tab==="assignments";const [catalogWidths,setCatalogWidth]=useSimpleColumnWidths("workcatalog2");const [matrixWidths,setMatrixWidth]=useSimpleColumnWidths("workmatrix2");const resizeCatalog=useEdgeResize(setCatalogWidth,40);const resizeMatrix=useEdgeResize(setMatrixWidth,40);const gridTemplate=(defaults:string[],widths:Record<string,number>)=>defaults.map((d,i)=>widths[i]?`${widths[i]}px`:d).join(" ");const catalogColumns={gridTemplateColumns:gridTemplate(["56px","minmax(240px,1fr)","minmax(300px,1.2fr)",...shownCompanies.map(()=>"92px")],catalogWidths)};const matrixColumns={gridTemplateColumns:gridTemplate(["56px","minmax(220px,1fr)","minmax(210px,.75fr)","170px",...shownCompanies.map(()=>"88px")],matrixWidths)};const selectedCompanies=new Set((form.assignCompanyIds||"").split(",").filter(Boolean).map(Number));const toggleAssignCompany=(id:number,checked:boolean)=>{const next=new Set(selectedCompanies);checked?next.add(id):next.delete(id);setForm({...form,assignCompanyIds:[...next].join(",")})};const filteredCatalogItems=items.filter(item=>(catalogCompanyFilter==="all"&&catalogEmployeeFilter==="all")||assignments.some(a=>a.work_definition_id===item.id&&(catalogCompanyFilter==="all"||String(a.company_id)===catalogCompanyFilter)&&(catalogEmployeeFilter==="all"||String(a.employee_id)===catalogEmployeeFilter)));const rows=assignments.filter(a=>catalogCompanyFilter==="all"||String(a.company_id)===catalogCompanyFilter).map(a=>({key:`${a.work_definition_id}:${a.employee_id}`,workId:a.work_definition_id,employeeId:a.employee_id,title:a.title,description:a.description,frequency:a.frequency,employeeName:a.employee_name})).filter((row,index,list)=>list.findIndex(other=>other.key===row.key)===index);
