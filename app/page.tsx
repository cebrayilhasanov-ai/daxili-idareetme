...
function CompanyStructureDialog({company,onClose}:{company:Company|null;onClose:()=>void}){
  ...
  const fields=(values:Record<string,string>,set:(next:Record<string,string>)=>void)=>{
    const reportsToSelected=new Set((values.reportsTo||"").split("/").map(s=>s.trim()).filter(Boolean));
    const toggleReportsTo=(title:string,checked:boolean)=>{const next=new Set(reportsToSelected);checked?next.add(title):next.delete(title);set({...values,reportsTo:Array.from(next).join("/")})};
    return <>
      ...
      <div className="field structurereportsto" key="reportsTo"><span>Tabe olduğu (bir neçə seçim mümkündür)</span><div className="fixedcompanies reportstopicker"><div>{items.filter(i=>String(i.id)!==values.editId).map(i=><label key={i.id}><input type="checkbox" checked={reportsToSelected.has(i.title)} onChange={e=>toggleReportsTo(i.title,e.target.checked)}/><span>✓</span>{i.title}</label>)}{!items.filter(i=>String(i.id)!==values.editId).length&&<small>Hələ başqa vəzifə yoxdur — bu, ən yuxarı vəzifə olacaq.</small>}</div></div></div>
    </>;
  };
  const create=async()=>{
  ...
  return <Dialog open={Boolean(company)} onOpenChange={v=>!v&&onClose()}><DialogContent className="businessdialog structuredialog" resizable>{company&&<FormShell title={`${company.name} — Təşkilati struktur`} desc="Şöbələr, vəzifələr və tabeçilik münasibətləri." formClass="structureform">
    ...
    {loading?<div className="loading">Yüklənir...</div>:<div className="tasktablewrap"><table className="tasktable structuretable"><thead><tr>
      ...
      return <tr key={item.id}>
        {unmerged?<td className="structuredeptcell" style={{background:deptColor(item.department)}}>{item.department}</td>:firstOfGroup&&<td rowSpan={rowSpan} className="structuredeptcell" style={{background:deptColor(item.department)}}>{item.department}</td>}
